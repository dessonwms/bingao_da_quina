  ...
  registrationBlocked: async (request, response, next) => {
    try {
      ...
      if (bingo.status === 'FINALIZADO') {
        return response.render('betting/blocked', {
          message: 'No momento não é possível cadastrar novas apostas! Aguarde até a liberação do próximo Bingão.'
        });
      }
      ...
      if (bingo.released_bets === 'C') {
        return response.render('betting/blocked', {
          message: 'O período de cadastro de apostas na atual edição do bingão já foi finalizada.'
        });
      }
