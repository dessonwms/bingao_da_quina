...
const BetsModel = {
  ...
  async create(data, sellerId) {
    ...
    RETURNING id
    `;
      const values = [data.userId, data.bingoId, sellerId, data.number[0], data.number[1], data.number[2], data.number[3], data.number[4], data.number[5], data.number[6], data.number[7], data.number[8], data.number[9], data.payment_status];
